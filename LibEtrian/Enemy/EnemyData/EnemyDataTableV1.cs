...
/// <summary>
/// The EO3 and EO4 version of the enemy data table.
/// </summary>
public class EnemyDataTableV1 : List<EnemyDataTableV1.EnemyDataV1>
{
  public EnemyDataTableV1(string path)
  {
    var data = File.ReadAllBytes(path);
    if (data.Length % EnemyDataV1.Length != 0)
    {
      throw new InvalidDataException($"EnemyDataTableV1 length is not cleanly divisible by the " +
                                     $"entry length (0x{EnemyDataV1.Length:X2}).");
    }
    AddRange(data
      .Split(EnemyDataV1.Length)
      .Select(e => new EnemyDataV1(e)));
  }
