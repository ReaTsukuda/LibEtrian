...
/// <summary>
/// The EOU version of the enemy data table.
/// </summary>
public class EnemyDataTableV2 : List<EnemyDataTableV2.EnemyDataV2>
{
  public EnemyDataTableV2(string path)
  {
    var data = File.ReadAllBytes(path);
    if (data.Length % EnemyDataV2.Length != 0)
    {
      throw new InvalidDataException($"EnemyDataTableV2 length is not cleanly divisible by the " +
                                     $"entry length (0x{EnemyDataV2.Length:X2}).");
    }
    AddRange(data
      .Split(EnemyDataV2.Length)
      .Select(e => new EnemyDataV2(e)));
  }
