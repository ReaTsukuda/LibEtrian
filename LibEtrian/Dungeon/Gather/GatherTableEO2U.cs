...
/// <summary>
/// The EO2U version of itemPoint.tbl, which controls gather points.
/// </summary>
public class GatherTableEO2U : List<GatherTableEO2U.GatherPointEO2U>
{
  public GatherTableEO2U(string path)
  {
    var data = File.ReadAllBytes(path);
    if (data.Length % GatherPointEO2U.Length != 0)
    {
      throw new InvalidDataException($"GatherTableEO2U length is not cleanly divisible by the " +
                                     $"entry length (0x{GatherPointEO2U.Length:X2}).");
    }
    AddRange(data
      .Split(GatherPointEO2U.Length)
      .Select(e => new GatherPointEO2U(e)));
  }
  
  /// <summary>
  /// An individual gather point defined in itemPoint.tbl.
  /// </summary>
  public class GatherPointEO2U
  {
    /// <summary>
    /// How long each gather point entry is.
    /// </summary>
    public const S32 Length = 0x70;
